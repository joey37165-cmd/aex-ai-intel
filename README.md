...
当前本地版本文件为 `config/templates/telegram.html`（实时情报）和 `config/templates/telegram_digest.html`（日报/周报共用），以及 `config/prompts/intelligence_filter.md`。实时消息会使用 AI 生成的 `display_title`，统一为中文并移除标题表情；链接行显示 `🔗 阅读原文` 和 `𝕏 Aex0x0`。修改模板和 Prompt 不需要改业务流程代码。
