...
知识库按钮和自动过期治理当前仍不实现；Email、X 适配器已实现但默认关闭，Langfuse Prompt 已支持远程拉取和本地回退。模板管理前端已连接带认证的 FastAPI 管理接口，SQLite 保存草稿、发布版本和历史记录，Worker 只读取已发布版本并保留仓库文件回退。它们按 README 的分期计划逐步启用。不要为了提前支持其他功能引入无法在小服务器上维护的基础设施。
